...
let lookup = [| (* Stolen directly out of the LAME VbrTag.c file (I don't know how to do CRCs! Why am I doing this?!) *)
...
let update_byte gnu crc =
	let temp = crc lxor gnu in
	(crc lsr 8) lxor lookup.(temp land 0xFF)
;;
let update_reference gnu crc_ref =
	let temp = !crc_ref lxor gnu in
	crc_ref := (!crc_ref lsr 8) lxor lookup.(temp land 0xFF)
;;

let create str initial =
	let crc_ref = ref initial in (* 0 for the LAME header *)
	for i = 0 to String.length str - 1 do
		update_reference (Char.code str.[i]) crc_ref
	done;
	!crc_ref
;;
...
(*
###########################################
## Updates a CRC value with another byte ##
sub updateCRC {
	my ($value, $crc) = @_;
	my $temp = $crc ^ $value;
	$crc = ($crc >> 8) ^ $mp3::CRClookup[$temp & 255];
	return $crc;
}

#################################
## Create a CRC given a string ##
sub createCRC {
	my ($string, $initialValue) = @_;
	my $crc = $initialValue; # 0 for the header thingie
	foreach my $char (split //, $string) {
		$crc = mp3::updateCRC(ord($char), $crc);
	}
	return $crc;
}
*)
