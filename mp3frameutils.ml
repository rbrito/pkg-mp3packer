...
(*				let (bits, value) = Hashtbl.find ht_encode (abs v, abs w, abs x, abs y) in*)
	...
	| M1_frame_data m when m.m1_header.header_channel_mode = ChannelMono -> (
		...
		(* Parts 2,3 *)
		(*************)
		let out_q = String.create 2881 in
		let r = (out_q, 0) in
	...
	| M1_frame_data m -> (
		...
		(* Parts 2,3 *)
		(*************)
		let out_q = String.create 2881 in (* This is the size of a padded 640kbps frame @ 32KHz (this will cover freeformat nicely) *)
		let r = (out_q, 0) in
...
*)
	...
	| M2_frame_data m when m.m2_header.header_channel_mode = ChannelMono -> (
		...
		let out_q = String.create 5761 in (* Length of a freeformat 640kbps frame @ 8khz (in case anybody wants to do that...) *)
		let r = (out_q, 0) in
